...
export default function EditInvoicePage() {
    ...
    const fetchInvoice = async () => {
      try {
        ...
        const matched = allClients.find(
          (c: any) => c.email?.toLowerCase() === (data.user_email || "").toLowerCase()
        );
        ...
        setFormData({
          ...
          legalName: data.legal_name || matched?.legalName || matched?.name || "",
        });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(e);
      alert("Failed to update invoice: " + (e instanceof Error ? e.message : String(e)));
      setIsSubmitting(false);
    }
  };

  if (loading) return <div className="flex justify-center items-center h-64"><Loader size="sm" /></div>;
  if (error) return <div className="text-center text-red-400 py-10">{error}</div>;

  return (
    <div className="space-y-6 pb-20">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/user/invoices" className="p-2 hover:bg-black/10 dark:hover:bg-white/10 rounded-full transition-colors text-gray-400 hover:text-foreground">
        ...
        <div className="space-y-8">
          ...
          <div className="relative z-50 bg-transparent border border-border rounded-3xl p-6 space-y-4">
            ...
            <div className="space-y-1">
              ...
              <ClientCombobox name="clientId" value={String(formData.clientId || "")} onChange={handleFormChange} clients={clients} loading={clientsLoading} />
            </div>
            ...
            {showNewClientForm && (
              <div className="p-4 bg-black-500/10 border border-border rounded-xl space-y-3">
                ...
                <textarea name="address" placeholder="Billing Address (Optional)" value={newClientData.address} onChange={handleNewClientChange}
                  className="w-full bg-transparent border border-border rounded-lg px-4 py-2 outline-none focus:border-brand-500 transition-colors text-sm resize-none" rows={2} />
            ...
            <div className="border-t border-border mt-6 pt-4 space-y-3">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <input type="number" name="discount" value={formData.discount || ""} onChange={handleFormChange} min="0"
                  className="w-24 bg-transparent border border-border rounded-lg px-3 py-1 outline-none focus:border-brand-500 transition-colors text-right text-sm" placeholder="0.00" />
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-gray-400">Tax Rate (%)</span>
                <input type="number" name="taxRate" value={formData.taxRate || ""} onChange={handleFormChange} min="0" max="100"
                  className="w-24 bg-transparent border border-border rounded-lg px-3 py-1 outline-none focus:border-brand-500 transition-colors text-right text-sm" placeholder="0" />
              </div>
              {formData.taxRate > 0 && (
                <div className="flex justify-between items-center text-sm">
              ...
              <div className="flex justify-between items-center text-lg font-semibold border-t border-border pt-3">
                <span>Total</span>
              ...
              <div className="flex justify-between items-center text-xl font-bold border-t border-border pt-3 text-foreground">
                <span>Total Due</span>
